...
@section('content')
    ...
    <div class="welcome-text">
        ...
        <span>{{ ___("Don't have an account?") }} <a href="{{ route('register') }}">{{ ___('Sign Up Now!') }}</a></span>
    </div>
    ...
    <form action="#" method="post" accept-charset="UTF-8">
        ...
        <div class="d-flex justify-content-between">
            ...
            <a href="{{ route('password.request') }}" class="forgot-password">{{ ___('Forgot Password?') }}</a>
        </div>
        <button class="button full-width button-sliding-icon ripple-effect margin-top-10" name="submit"
                type="submit">{{ ___('Login') }} <i class="icon-feather-arrow-right"></i></button>
    </form>
    @if(demo_mode())
        <table class="margin-top-30 margin-bottom-30 text-center" width="100%" border="1" style="border:1px solid #dee2e6">
            <tr>
                <th></th>
                <th>{{ ___('Username') }}</th>
                <th>{{ ___('Password') }}</th>
